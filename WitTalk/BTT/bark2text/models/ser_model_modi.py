...
# __all__ = ['Ser_Model'w]
class Ser_Model_cat_coc(nn.Module):
    def __init__(self):
        super(Ser_Model_cat_coc, self).__init__()
        ...
        self.post_att_layer_2 = nn.Linear(128, 4)
    ...
    def forward(self, audio_spec, audio_mfcc, audio_wav , audio_coc):      
        ...
        output_att = self.post_att_layer_2(audio_att_1) # [batch, 4] 
...
class Ser_Model_without_spec_mfcc(nn.Module):
    def __init__(self):
        super(Ser_Model_without_spec_mfcc, self).__init__()
    ...
    def forward(self, audio_spec, audio_mfcc, audio_wav , audio_coc):      
        ...
        #audio_att_d_1 = self.post_att_dropout(audio_att) # [batch, 384] 
        #audio_att_1 = F.relu(self.post_att_layer_1(audio_att_d_1), inplace=False) # [batch, 128] 
        output_att = self.post_att_layer_2(audio_wav_p) # [batch, 4] 
...
class Ser_Model_no_mfcc(nn.Module):
    def __init__(self):
        super(Ser_Model, self).__init__()
        ...
        # Spectrogram + coc  
        self.post_multi_att_layer = nn.Linear(256 , 149)
    ...
    def forward(self, audio_spec, audio_mfcc, audio_wav , audio_coc):      
        ...
        multi_f = torch.cat([audio_spec_p , audio_coc_p] , dim=-1)  #[batch, 384]
    
        multi_d = self.post_spec_dropout(multi_f)
        multi_p = F.relu(self.post_multi_att_layer(multi_d) , inplace=False)# [batch, 256]
        multi_p = multi_p.reshape(multi_p.shape[0] , 1, -1) #[batch , 1 , 149]
        ...
        #audio_att = torch.cat([audio_spec_p, audio_mfcc_p, audio_wav_p], dim=-1)  # [batch, 384] 
        #정확도 애매하게 내려감 
        audio_att = torch.cat([audio_spec_p, audio_wav_p , audio_coc_p], dim=-1)  # [batch, 512] 
