...
def extract_features(speaker_files, features, params , coc_params):
    ...
    for speaker_id in speaker_files.keys():
        ...
        convention_fea = list()
        for wav_path, emotion , species in tqdm(speaker_files[speaker_id]):
            ...
            for x in segments:
                ...
                # 68,7
                conven_features = get_conven_features(x ,  species)
                ...
                # 임의로 정한 길이 342번째 segment에서 메모리의 상한선인 15G가 죽는다.
               # Post process
