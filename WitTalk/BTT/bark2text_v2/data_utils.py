...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data, num_classes=13):
        ...
        self.conven_fea = data['seg_conven_fea']
        # self.target = target
    ...
    def __getitem__(self, index):
        sample = {
            ...
            'seg_conven_fea' : self.conven_fea[index]
            } 
        return sample
    ...
    def get_second_third_preds(self, preds):
   
        soted_indices = np.argsort(preds)
        # return second largest , third_largest index
        return soted_indices[-2] , soted_indices[-3]
...
class TestDataset(torch.utils.data.Dataset):
    ...
    def get_preds(self, seg_preds):
        ...
        # return second largest , third_largest index
        # 104 , 13
        second_preds , third_preds = [] , []
        for pred in preds:
            sorted_indices = np.argsort(pred)
            second_preds.append(sorted_indices[-2])
            third_preds.append(sorted_indices[-3])

        preds = np.argmax(preds, axis=1)

        return preds , sorted_indices[-2] , sorted_indices[-3]


    def weighted_accuracy(self, utt_preds , utt_preds2 , utt_preds3):
        """
        ...
        acc3 = ((self.actual_target == utt_preds).sum() + (self.actual_target == utt_preds2).sum() + \
            (self.actual_target == utt_preds3).sum()) / self.n_actual_samples
    ...
    def unweighted_accuracy(self, utt_preds):
        ...
        for c in range(self.num_classes):
            ...
            if (self.actual_target == c).sum() > 0:    
                ...
                n_classes += 1
                class_acc += class_pred
                n_class_acc[c] = class_pred


        return class_acc / n_classes, n_class_acc
...
class SERDataset:
    def __init__(self, features_data, num_classes = 13,
        ...
        for speaker_id in features_data.keys():
            ...
            if train_mfcc_data is None:
                ...
                train_coc_data = features_data[speaker_id]['seg_coc'].astype(np.float32)
                train_conven_data = features_data[speaker_id]['conven_feature'].astype(np.float32)
                ...
                train_audio_data = np.concatenate((train_audio_data, 
                                            ...
                                            axis=0)
                train_coc_data = np.concatenate((train_coc_data, 
                                            features_data[speaker_id]['seg_coc'].astype(np.float32) ),
                                            axis=0)
                train_conven_data = np.concatenate((train_conven_data,
                                                   features_data[speaker_id]['conven_feature'].astype(np.float32)),
                                                axis = 0)
        ...
        if mode == 'train':                               
            ...
            self.train_coc_data = train_coc_data
            self.train_seg_labels = train_seg_labels
            self.train_labels    = train_labels
            self.train_conven_data = train_conven_data

        
        #get validation spectrograms
        else:
            self.val_spec_data  = features_data[speaker_id]['seg_spec'].astype(np.float32)
            self.val_mfcc_data  = features_data[speaker_id]['seg_mfcc'].astype(np.float32)
            self.val_audio_data  = features_data[speaker_id]['seg_audio'].astype(np.float32)
            self.val_coc_data = features_data[speaker_id]['seg_coc'].astype(np.float32)
            self.val_seg_labels = features_data[speaker_id]['seg_label'].astype(np.int64)
            self.val_labels     = features_data[speaker_id]['utter_label'].astype(np.int64)
            self.val_num_segs   = features_data[speaker_id]['seg_num']
            self.val_conven_data = features_data[speaker_id]['conven_feature'].astype(np.int64)
        ...
        if oversample == True and mode == 'train':
        ...
        if mode == 'train':
            ...
            self.train_data["seg_coc"] =  self.train_coc_data
            self.train_data["seg_label"] = self.train_seg_labels
            self.train_data["seg_conven_fea"] = self.train_conven_data
        ...
        #convert normalized spectrogram to 3 channel image, apply AlexNet image pre-processing
            ...
            self.val_data["seg_coc"] = self.val_coc_data
            ...
            self.val_data["seg_conven_fea"]  = self.val_conven_data
    ...
    def _normalize(self, scaling):
        ...
        if self.mode == 'train':
            ...
            self.train_coc_data = self.train_coc_data.transpose(0,2,1)
            self.train_spec_data = rearrange(self.train_spec_data)

        else:
            nsegs = self.val_spec_data.shape[0]
            nch   = self.val_spec_data.shape[1]
            nfreq = self.val_spec_data.shape[2]
            ntime = self.val_spec_data.shape[3]
            rearrange = lambda x: x.transpose(1,0,3,2).reshape(nch,-1,nfreq)

            self.val_spec_data   = rearrange(self.val_spec_data)
            self.val_coc_data = self.val_coc_data.transpose(0,2,1)
        ...
        for ch in range(nch):
            ...
            if self.mode == 'train':
                ...
                self.train_coc_data[ch] = scaler.transform(self.train_coc_data[ch])
            else:
                scale_values = scaler.fit(self.val_spec_data[ch])
                self.val_spec_data[ch] = scaler.transform(self.val_spec_data[ch])
                self.val_coc_data[ch] = scaler.transform(self.val_coc_data[ch])
