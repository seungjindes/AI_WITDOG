...
class DataPipeline:
    ...
    def execute(self, x , sr ,species, emotion , sound , target , age , num):
        data_features = extract_features(x,sr , species, emotion , sound , target , age , num , self.features, self.params , self.coc_params)
...
def postprocess(results, output_name, target ):
    """
    ...
    result_val = int(cls[1])  # Convert to integer for comparison

    emotion = None
    for key, value in emot_map.items():
        if value == result_val:
            if value == 1:
                if target == 'oth' : emotion = '흥분(화남)'
                elif target == 'own' : emotion = '행복'
            elif value == 2: 
                if target == 'oth' : emotion = '호기심'
                elif target == 'own' : emotion = '요구'
            else : emotion = key
...
def get_emotion_using_triton(input_data , target ,url = "172.17.0.1" , port = "8005" , concurrency = 1):
    ...
    obj = postprocess(infer_response , output_name[0] , target)
    return obj["emo"]
